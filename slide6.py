...
VIDEO_PATH = "video/"+VIDEO_SUB_PATH+"/slide6"+name_suffix+".mp4"
...
logo = ImageClip(OBJ_PATH).resize(0.9/size_factor)
...
txt = TextClip("Living Room", font=bold_font,color=font_color, fontsize=font_size, method='label', interline=interline, kerning=kerning).set_pos(object_txt_position(TEMPLATE_PATH), relative=True).set_duration(DURATION  - 0.7).set_start(START_TIME + START_TIME).crossfadein(FADE_TIME + 0.25)
